...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        ...
        """
        # resize if table load factor is greater than or equal to 0.5
        if self.table_load() >= 0.5:
            new_capacity = self.get_capacity() * 2
            self.resize_table(new_capacity)
        
        # find hash key 
        # hash key only to find spot nothing else 
        initial_index = self._hash_function(key) % self._capacity 
        # initial_index = hash_key

        for i in range(self.get_capacity()):
            hash_key = (initial_index + i ** 2) % self._capacity
            node = self._buckets[hash_key]
            if node is None or node.is_tombstone == True:
    ...
    def get(self, key: str) -> object:
        ...
        for i in range(self.get_capacity()):
            hash_key = (initial_index + i ** 2) % self._capacity
            node = self._buckets[hash_key]
            if node is not None and node.is_tombstone == False and node.key == key:
                return node.value
        return None
    ...
    def remove(self, key: str) -> None:
        ...
        for i in range(self.get_capacity()):
            ...
            node = self._buckets[hash_key]
            if node is not None and node.is_tombstone == False and node.key == key:
                # = set is_tombstone to True meaning data is removed
                node.is_tombstone = True
                # removed, so decrease size 
                self._size -= 1
    ...
    def get_keys_and_values(self) -> DynamicArray:
        ...
        for i in range(self._capacity):
            ...
            # check if bucket has live data in it 
            if bucket is not None and bucket.is_tombstone == False:
                results_array.append((bucket.key, bucket.value))
    ...
    def __next__(self):
        """
        Return the next item in hash map based on 
